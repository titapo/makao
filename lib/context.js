...
function Context(rootNode)
{
    ...
    this.view = new View('view', this.model, rootNode, new Path([]));
    ...
    this.store = function()
    {
        try
        {
            ...
            var p = storage.path;
            this.feedback(p.display());
            var node = p.navigate(this.tree.root);
    ...
    this.processStorages = function(storageDict)
    {
        ...
        for (var name in storageDict)
        {
            ...
            {
                case "browser-local":
                    storages[name].storage = new BrowserStorage(this.tree, storageConf.target, this.entityFactory);
                    break;
                case "plain":
                    storages[name].storage = new PlainStorage(this.tree, this.entityFactory);
                    break;
                default:
            ...
            if (storageConf["auto_load"] === "true")
                storages[name].storage.loadNode();
        }
    }

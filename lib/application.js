...
StorageOperationHandler.prototype = {
    ...
    changeStorage: function()
    {
        ...
        form.submit = function(values)
        {
            try
            {
                this.context.selectStorage(this.inputs['storage'].value);
                DisplayActualNode();
                this.context.feedback("Storage changed");
                return true;
            }
            catch(err)
            {
    ...
    actions.set("node-actions", function(node)
    {
        ...
        */
        ...
        if (g_context.hasStorage())
        {
            ...
            menu.add(actionLink("load", new Action(new Callback(g_handler, g_handler.load), "load").display()));
            menu.add(actionLink("store", new Action(new Callback(g_handler, g_handler.store), "store").display()));
            menu.add(actionLink("clear", new Action(new Callback(g_handler, g_handler.clear), "clear").display()));
            menu.add(actionLink("change storage", new Action(new Callback(g_handler, g_handler.changeStorage), "change-storage").display()));

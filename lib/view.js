...
View.prototype.display = function()
{
    ...
    for (var i = 0; i < result.content.length; ++i)
    {
        ...
        {
            var actionId = child.actions[j];
            console.log("  " + j + " actionId:" + actionId);
            var action_names = this.actionList.list[actionId]();

            for (var k = 0; k < action_names.length; ++k)
            {
                console.log("   create button for: " + action_names[k]);
                action_content += ActionButton(
                        this.remap(action_names[k]),

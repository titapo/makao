...
class View extends Observer
{
    ...
    _displayCurrentPath()
    {
        ...
        let root = this.tree.getEntity(new Path([]))
        ...
        this.path.process(
                function(node)
                {
                    ...
                    out += ActionButton(
                        base.name,
                        controller.getAction("go-to").display(Path.encode(path)));
                },
                root);
    ...
    updatePath(path)
    {
        // TODO(titapo) checking would be useful
        this.path = path;
        this.display();
    }
}
